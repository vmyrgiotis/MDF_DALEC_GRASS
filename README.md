...
This is a python package that implements an ecosystem model of grassland carbon (C) biogeochemistry [(DALEC-Grass)](https://www.sciencedirect.com/science/article/abs/pii/S0308521X2030768X). DALEC-Grass is integrated into a model-data fusion (MDF) framework. The MDF framework implements DALEC-Grass probabilistically and uses location-specific (1) weather time-series and soil data to drive the model and (2) earth observation (EO) time-series on vegetation leaf area, in order to (1) calibrate the model's parameters (2) validate model predictions and (3) estimate field-scale C cycling i.e. C in biomass,roots, soil, C lost to the atmosphere and C removed via grazing/cutting. 
